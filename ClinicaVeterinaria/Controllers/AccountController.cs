...
namespace ClinicaVeterinaria.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public AccountController(ApplicationUserManager userManager, ApplicationSignInManager signInManager, ClinicaVeterinariaContext context)
        {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            switch (result)
            {
                case SignInStatus.Success:
                    // Obtendo o usuário e suas Roles
                    ...
                    {
                        string url = Url.Action("Index", "Veterinarios", new { area = "Veterinarios" });
                        Session["UserArea"] = "Veterinarios";
                        return Redirect(url);

                    }
                    else if (roles.Contains("Secretario"))
                    {
                        Session["UserArea"] = "Secretarios";
                        return RedirectToAction("Index", "Secretarios", new { area = "Secretarios" }); // Redireciona para a página do Secretário
                    }
                    else if (roles.Contains("Proprietario"))
                    {
                        Session["UserArea"] = "Proprietarios";
                        return RedirectToAction("Index", "Proprietarios", new { area = "Proprietarios" }); // Redireciona para a página do Proprietário
                    }
                    else if (roles.Contains("Curioso"))
                    {
                        Session["UserArea"] = "Curiosos";
                        return RedirectToAction("Index", "Curiosos", new { area = "Curiosos" }); // Redireciona para a página do Curioso
                    }
                    // Se o usuário não tiver uma das roles definidas, redireciona para uma página padrão
                ...
                default:
                    ...
                    // Tenta criar o usuário como Curioso
                    var newUser = new ApplicationUser { UserName = model.Email, Email = model.Email };
                    var userCreationResult = await UserManager.CreateAsync(newUser, model.Password);
                    if (userCreationResult.Succeeded)
                    {
                        // Atribui o papel "Curioso"
                        await UserManager.AddToRoleAsync(newUser.Id, "Curioso");

                        // Realiza o login do usuário recém-criado
                        await SignInManager.SignInAsync(newUser, model.RememberMe, false);

                        Session["UserArea"] = "Curiosos";
                        return RedirectToAction("Index", "Curiosos", new { area = "Curiosos" });
                    }
                    ModelState.AddModelError("", "Tentativa de login inválida.");
                    return View(model);
            }
